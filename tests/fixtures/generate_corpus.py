...
from corpus_builder import build_corpus, without_additive_v1_fields  # noqa: E402


def main() -> None:
    target = Path(__file__).resolve().parent / "session_schema_v1.json"
    with tempfile.TemporaryDirectory() as workspace:
        document = without_additive_v1_fields(asyncio.run(build_corpus(Path(workspace))))
