...
状态：M0–M7、H-01 ~ H-17、P-01 ~ P-01.4 已完成；Coding TUI 已支持可恢复、可滚动对话，审计日志已纳入运行诊断
...
| 编号 | 项 | 状态 | 说明 |
|---|---|---|---|
| P-01 | 应用层重建（TUI 前端） | Done | Worker RPC、Session/Task、Ink TUI、Provider profiles、Run 流式执行/取消/恢复、Approval/Skill/MCP/Tool 管理、Git 只读工具及 Artifact/Compaction/Subagent 配置均已接入；默认写入脱敏 `audit.jsonl`，`/doctor` 检查审计目标 |
...
| P-01.4 | 会话与模型选择 UX | Done | 可搜索 Session/Provider/Model picker、`/status` 与 `/doctor`；`/doctor` 同时检查 audit JSONL 文件或其最近可写父目录；继续复用既有 RPC，不扩协议 |
