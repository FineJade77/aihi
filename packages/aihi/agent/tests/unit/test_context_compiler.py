...
def test_context_compiler_bounds_large_tool_results_without_artifact_store() -> None:
    original = "head-" + "x" * 8_000 + "-tail"
    message = Message(
        role="user",
        content=(ToolResultBlock(tool_call_id="call-1", content=original),),
    )

    compiled = ContextCompiler(
        artifact_threshold_tokens=10,
    ...
    projected = compiled.messages[0].tool_results[0]
    assert projected.content.startswith("head-")
    assert projected.content.endswith("-tail")
    assert projected.metadata["context_projection"] == "truncated"
    assert projected.metadata["original_size_bytes"] == len(original.encode())
    assert message.tool_results[0].content == original
...
@pytest.mark.asyncio
async def test_compaction_never_splits_tool_call_and_result_pair() -> None:
    call = Message(
        role="assistant",
    ...
    assert compiled.messages[0].metadata["compaction"] == "rolling_summary"
...
def test_context_compiler_exposes_over_budget_projection_for_coordinator() -> None:
    compiled = ContextCompiler().compile(
...
def test_context_compiler_keeps_system_prompt_in_the_projection() -> None:
    compiled = ContextCompiler().compile(
        (),
...
async def test_l2_compaction_uses_injected_structured_summary_generator() -> None:
    ...
    compiled = await ContextCompiler(summary_generator=generator).compile_and_compact(
        messages,
        system_prompt="",
        tools=(),
    ...
    assert "keep the API stable" in compiled.messages[0].text_content
