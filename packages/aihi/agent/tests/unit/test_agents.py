...
def test_task_graph_spawns_and_round_trips_snapshot() -> None:
    ...
    assert graph.snapshot()["schema_version"] == 2
...
def test_task_graph_loads_a_v1_snapshot_with_legacy_workspace_data() -> None:
    graph = TaskGraph()
    root = graph.create_root(parent_run_id="run", objective="root", budget=_budget())
    snapshot = graph.snapshot()
    snapshot["schema_version"] = 1
    nodes = snapshot["nodes"]
    assert isinstance(nodes, dict)
    nodes[root.spec.task_id]["spec"]["workspace"] = {
        "root": "/legacy",
        "read_only": True,
        "allowed_paths": [],
    }
    ...
    assert restored.get(root.spec.task_id).spec.objective == "root"
