...
`rolling_summary_fallback`, so a compact model that quietly never works is visible.
"""
...
STRATEGY_MODEL = "rolling_summary_model"
STRATEGY_FALLBACK = "rolling_summary_fallback"
...
class ModelSummaryGenerator:
    ...
    async def generate(self, request: SummaryRequest) -> StructuredSummary:
        ...
        async def summarize(chunk: SummaryRequest) -> StructuredSummary:
            async with semaphore:
                try:
                    return self._parse(await self._ask(chunk), chunk)
                except Exception as error:  # noqa: BLE001 - degrade this chunk only.
                    degraded = await self.fallback.generate(chunk)
        ...
        summaries = await asyncio.gather(
            *(summarize(chunk) for chunk in self._chunks(request))
        )
    ...
    def _chunks(self, request: SummaryRequest) -> tuple[SummaryRequest, ...]:
        ...
        groups = group_tool_exchanges(request.omitted_messages)
        if not groups:
            return (request,)
...
def _render_messages(messages: tuple[Message, ...]) -> str:
    parts = [rendered for message in messages if (rendered := _render_message(message))]
...
def _render_message(message: Message) -> str:
    ...
    for block in message.content:
        if isinstance(block, TextBlock) and block.text.strip():
            ...
            blocks.append(
                f"tool_call id={block.id} name={block.name} input="
                f"{json.dumps(block.input, ensure_ascii=False, sort_keys=True)}"
            )
        elif isinstance(block, ToolResultBlock):
            status = "error" if block.is_error else "ok"
            blocks.append(
                f"tool_result call_id={block.tool_call_id} status={status}\n{block.content}"
            )
        elif isinstance(block, ImageBlock):
            blocks.append(f"[image media_type={block.media_type}]")
        elif isinstance(block, ThinkingBlock):
            # Provider reasoning is not durable task state and is intentionally
...
def _merge_summaries(
    ...
    return StructuredSummary(
        strategy=STRATEGY_FALLBACK if fallback_reasons else STRATEGY_MODEL,
        objective=next(
