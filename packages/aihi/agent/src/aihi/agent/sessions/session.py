...
from aihi.agent.policy import Approval, AuthorizationState, CapabilityLease
from aihi.agent.sessions.store import EventStore
from aihi.models import Message, ToolCallBlock, ToolResultBlock, decode_message, encode_message
    ...
    for event in events:
        if event.type in {
            ...
            if isinstance(raw, dict):
                messages.append(decode_message(dict(event.data)))
            ...
            messages.insert(
                ...
                decode_message(
                    {
                        "message_schema_version": event.data.get(
                            "summary_message_schema_version", 1
                        ),
                        "message": raw_summary,
                    }
                ),
...
class Session:
    ...
    def append_many(self, events: list[Event]) -> list[Event]:
        ...
        for event in events:
            ...
            raw = event.data.get("message")
            if isinstance(raw, dict):
                # Fail before the append-only store sees an unsupported
                # cross-distribution message envelope.
                candidate_messages.append(decode_message(dict(event.data)))
        ...
        for event in persisted:
            if event.type in {
                ...
                raw = event.data.get("message")
                if isinstance(raw, dict):
                    self._messages.append(decode_message(dict(event.data)))
            elif event.type == "compaction.created":
        ...
        return self.append_many(
            [
                ...
                Event(
                    ...
                    session_id=self.id,
                    run_id=run_id,
                    data=encode_message(repaired_message),
                ),
            ]
        )
