        ...
        *,
        ...
        with self._lock:
            ...
            self._sessions[session_id] = SessionInfo(
                ...
                head_seq=0,
                created_at=utc_now(),
                metadata=_json_copy(metadata, field="session metadata"),
    ...
    def append(self, session_id: str, expected_seq: int, events: list[Event]) -> list[Event]:
        with self._lock:
            ...
            for offset, event in enumerate(events, start=1):
                ...
                # The in-memory implementation must enforce the same durable
                # JSON boundary as SQLite; otherwise tests can hide payloads
                # that cannot be restored from the real store.
                data = _json_copy(event.data, field="event data")
                event = Event(
                    type=event.type,
                ...
                returned.append(_clone_event(saved))
...
class SQLiteEventStore:
    ...
    def create_session(
        ...
        *,
        ...
        payload = _json_dump(metadata, field="session metadata")
        with self._lock:
            try:
                ...
                for offset, event in enumerate(events, start=1):
                    ...
                    connection.execute(
                        ...
                        (
                            ...
                            _json_dump(saved.data, field="event data"),
                        ),
                    )
                    persisted.append(saved)
                connection.execute(
...
def _json_copy(value: object, *, field: str) -> dict[str, Any]:
    payload = json.loads(_json_dump(value, field=field))
