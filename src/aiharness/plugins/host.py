...
from aiharness.plugins.host_protocol import (
    PLUGIN_HOST_PROTOCOL_VERSION,
    make_request,
    validate_response,
)
from aiharness.plugins.trust import PluginTrustManager
...
class PluginHost:
    ...
    async def start(self) -> None:
        async with self._lock:
            ...
            command = list(
                self.worker_command
                or (self.python_executable, "-m", "aiharness.plugins.host_worker")
            )
            command.extend(
                (
                    "--root",
            ...
            pythonpath = [str(source_root), str(activated.root)]
