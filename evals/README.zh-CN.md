...
- `aihi_code_agent/` 保存 Coding Agent 任务，在隔离工作区执行，并使用隐藏测试、修改范围检查和导出的
  Harness Trace 评分。`v1/` 是能力 Benchmark；`context-v1/` 是成对的长 Session Cache/Compaction 门禁。

Schema 位于 `schemas/`。完整契约见
[`docs/EVALUATION.zh-CN.md`](../docs/EVALUATION.zh-CN.md) 和
[`docs/EVALUATION.md`](../docs/EVALUATION.md)。

数据集目录使用 `v1`、`v2` 等版本，也可以使用 `context-v1` 这类用途前缀。如果 Fixture 或 Schema
改变了既有案例的含义，必须创建新的数据集
版本；不要为了让实现通过而改写冻结案例。
